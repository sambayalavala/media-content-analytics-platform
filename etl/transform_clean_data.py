...
# ----- 1) PROCESS NEWS (Kaggle Yahoo dataset) -----
...
# Basic cleanup: drop completely empty columns, normalize column names
...
# Standardize names to lower-case for easier mapping
news_cols = {c:c.lower() for c in news.columns}
news = news.rename(columns=news_cols)

# Map expected columns - handle different names
# Common raw names: title -> headline, content -> news_text, label -> sentiment
...
# Ensure columns exist (create with default if missing)
for col in ['headline','news_text','sentiment','source','category','publish_date']:
    if col not in news.columns:
        news[col] = pd.NA

# Convert sentiment to int if possible, else map text labels
...
# common names: video_id, title, channel_title, views, likes, comment_count, category, publish_date
